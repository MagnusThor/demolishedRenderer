...
// Orbital Flight by Magnus Thor & Jerome Liard, September 2020
// License Creative Commons Attribution-NonCommercial-ShareAlike 3.0 Unported License.

// Mouse to look around ( may work ) . The camera switches every 30 seconds, there are currently 8 different paths.
...
// Disclaimer: important physical constants use garbage values.
...
float warp_nl( float nl, float cut ) { 
	return max( 0.0, ( nl - cut ) ) / ( 1.0 - cut ); 
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	if ( ss < 0.7 )
	{
		...
		fragColor.rgb = vec3(0.5529, 0.2196, 0.2196);
		for ( int i = 0; i < 2; i++ )
		{
			for ( int j = 0; j < 2; j++ )
			{
