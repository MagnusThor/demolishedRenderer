...
vec3 light(in vec3 p, in vec3 dir, in vec3 n, in float hid) {
	...
	float ao=calcAO(p,n);
	float diff=max(0.,dot(lightdir,-n))*sh*1.;
	vec3 amb=max(.5,dot(dir,-n))*.35*vec3(.75,.65,.6);
	vec3 r = reflect(lightdir,n);
	float spec=pow(max(0.,dot(dir,-r))*sh,15.)*.65;
	vec3 col;
	...
	col=col*ao*(amb+diff*vec3(1.,.97,.93))+spec*vec3(1.,.97,.93);	
...

{
	...
	vec3 backg=vec3(max(0.,-dir.y+.6))*vec3(.75,.65,.6)*.5*max(0.4,l);
	...
	vec3 lglow=vec3(1.,.97,.93)*pow(l,25.)*.5;
	col+=glow*(.5+l*.5)*vec3(1.,.97,.93)*.7;
...
void main()
{
	...
	color=pow(color,vec3(1.35))*1.5;
	color=mix(vec3(length(color)),color,.8);
