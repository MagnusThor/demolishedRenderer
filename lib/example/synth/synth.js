...
exports.Config = exports.Synth = exports.Song = exports.Channel = exports.Instrument = exports.EnvelopeSettings = exports.FilterSettings = exports.FilterControlPoint = exports.HarmonicsWave = exports.SpectrumWave = exports.Operator = exports.Pattern = exports.Note = exports.makeNotePin = exports.clamp = void 0;
const SynthConfig_1 = require("./SynthConfig");
...
class Note {
    ...
    clone() {
        const newNote = new Note(-1, this.start, this.end, SynthConfig_1.Config.noteSizeMax);
    ...
    fromJsonObject(patternObject, song, channel, importedPartsPerBeat, isNoiseChannel) {
        if (song.patternInstruments) {
            if (Array.isArray(patternObject["instruments"])) {
                ...
                const instrumentCount = clamp(SynthConfig_1.Config.instrumentCountMin, song.getMaxInstrumentsPerPatternForChannel(channel) + 1, instruments.length);
        ...
        if (patternObject["notes"] && patternObject["notes"].length > 0) {
            const maxNoteCount = Math.min(song.beatsPerBar * SynthConfig_1.Config.partsPerBeat, patternObject["notes"].length >>> 0);
            ...
            for (let j = 0; j < patternObject["notes"].length; j++) {
                ...
                for (let k = 0; k < noteObject["points"].length; k++) {
                    ...
                    const time = Math.round((+pointObject["tick"]) * SynthConfig_1.Config.partsPerBeat / importedPartsPerBeat);
                ...
                const maxPitch = isNoiseChannel ? SynthConfig_1.Config.drumCount - 1 : SynthConfig_1.Config.maxPitch;
...
class SpectrumWave {
    ...
    reset(isNoiseChannel) {
        for (let i = 0; i < SynthConfig_1.Config.spectrumControlPoints; i++) {
            if (isNoiseChannel) {
    ...
    markCustomWaveDirty() {
        const hashMult = Synth.fittingPowerOfTwo(SynthConfig_1.Config.spectrumMax + 2) - 1;
        let hash = 0;
...
class SpectrumWaveState {
    ...
    getCustomWave(settings, lowestOctave) {
        ...
        const waveLength = SynthConfig_1.Config.spectrumNoiseLength;
        ...
        for (let i = 0; i < SynthConfig_1.Config.spectrumControlPoints + 1; i++) {
            const value1 = (i <= 0) ? 0 : settings.spectrum[i - 1];
            const value2 = (i >= SynthConfig_1.Config.spectrumControlPoints) ? settings.spectrum[SynthConfig_1.Config.spectrumControlPoints - 1] : settings.spectrum[i];
...
class HarmonicsWave {
    ...
    reset() {
        for (let i = 0; i < SynthConfig_1.Config.harmonicsControlPoints; i++) {
            this.harmonics[i] = 0;
        }
    ...
    markCustomWaveDirty() {
        const hashMult = Synth.fittingPowerOfTwo(SynthConfig_1.Config.harmonicsMax + 2) - 1;
        let hash = 0;
...
class HarmonicsWaveState {
    ...
    getCustomWave(settings, instrumentType) {
        ...
        const harmonicsRendered = (instrumentType == 7 /* InstrumentType.pickedString */) ? SynthConfig_1.Config.harmonicsRenderedForPickedString : SynthConfig_1.Config.harmonicsRendered;
        const waveLength = SynthConfig_1.Config.harmonicsWavelength;
        const retroWave = (0, SynthConfig_1.getDrumWave)(0, null, null);
        if (this.wave == null || this.wave.length != waveLength + 1) {
            this.wave = new Float32Array(waveLength + 1);
        }
        const wave = this.wave;
        ...
        for (let harmonicIndex = 0; harmonicIndex < harmonicsRendered; harmonicIndex++) {
            ...
            let controlValue = harmonicIndex < SynthConfig_1.Config.harmonicsControlPoints ? settings.harmonics[harmonicIndex] : settings.harmonics[SynthConfig_1.Config.harmonicsControlPoints - 1];
            ...
            const normalizedValue = controlValue / SynthConfig_1.Config.harmonicsMax;
            let amplitude = Math.pow(2, controlValue - SynthConfig_1.Config.harmonicsMax + 1) * Math.sqrt(normalizedValue);
            if (harmonicIndex < SynthConfig_1.Config.harmonicsControlPoints) {
        ...
        for (let i = 0; i < wave.length; i++)
            wave[i] *= mult;
        (0, SynthConfig_1.performIntegral)(wave);
...
class FilterControlPoint {
    ...
    getLinearGain(peakMult = 1.0) {
        const power = (this.gain - SynthConfig_1.Config.filterGainCenter) * SynthConfig_1.Config.filterGainStep;
    ...
    toCoefficients(filter, sampleRate, freqMult = 1.0, peakMult = 1.0) {
        const cornerRadiansPerSample = 2.0 * Math.PI * Math.max(SynthConfig_1.Config.filterFreqMinHz, Math.min(SynthConfig_1.Config.filterFreqMaxHz, freqMult * this.getHz())) / sampleRate;
    ...
    getVolumeCompensationMult() {
        const octave = (this.freq - SynthConfig_1.Config.filterFreqReferenceSetting) * SynthConfig_1.Config.filterFreqStep;
        const gainPow = (this.gain - SynthConfig_1.Config.filterGainCenter) * SynthConfig_1.Config.filterGainStep;
        switch (this.type) {
            case 0 /* FilterType.lowPass */:
                const freqRelativeTo8khz = Math.pow(2.0, octave) * SynthConfig_1.Config.filterFreqReferenceHz / 8000.0;
            ...
            case 2 /* FilterType.peak */:
                const distanceFromCenter = octave + Math.log2(SynthConfig_1.Config.filterFreqReferenceHz / 2000.0);
...
class FilterSettings {
    ...
    convertLegacySettings(legacyCutoffSetting, legacyResonanceSetting, legacyEnv) {
        ...
        const envDecays = (legacyEnv.type == 3 /* EnvelopeType.flare */ || legacyEnv.type == 4 /* EnvelopeType.twang */ || legacyEnv.type == 8 /* EnvelopeType.decay */ || legacyEnv.type == 0 /* EnvelopeType.noteSize */);
        ...
        if (legacyEnv.type == 1 /* EnvelopeType.none */ && !resonant && cutoffAtMax) {
            ...
            const legacyFilter = new filtering_1.FilterCoefficients();
            legacyFilter.lowPass1stOrderSimplified(legacyRadians);
            const response = new filtering_1.FrequencyResponse();
...
class EnvelopeSettings {
    ...
    toJsonObject() {
        const envelopeObject = {
            ...
            "envelope": SynthConfig_1.Config.envelopes[this.envelope].name,
        };
    ...
    fromJsonObject(envelopeObject) {
        this.reset();
        let target = SynthConfig_1.Config.instrumentAutomationTargets.dictionary[envelopeObject["target"]];
        if (target == null)
            target = SynthConfig_1.Config.instrumentAutomationTargets.dictionary["noteVolume"];
        this.target = target.index;
        let envelope = SynthConfig_1.Config.envelopes.dictionary[envelopeObject["envelope"]];
        if (envelope == null)
...
class Instrument {
    constructor(isNoiseChannel) {
        ...
        for (let i = 0; i < SynthConfig_1.Config.operatorCount; i++) {
            this.operators[i] = new Operator(i);
        }
        for (let i = 0; i < SynthConfig_1.Config.drumCount; i++) {
    ...
    convertLegacySettings(legacySettings) {
        ...
        const legacyFilterCutoffRange = 11;
        const cutoffAtMax = (legacyCutoffSetting == legacyFilterCutoffRange - 1);
        if (cutoffAtMax && legacyFilterEnv.type == 2 /* EnvelopeType.punch */)
            legacyFilterEnv = SynthConfig_1.Config.envelopes.dictionary["none"];
        const carrierCount = SynthConfig_1.Config.algorithms[this.algorithm].carrierCount;
        let noCarriersControlledByNoteSize = true;
        let allCarriersControlledByNoteSize = true;
        let noteSizeControlsSomethingElse = (legacyFilterEnv.type == 0 /* EnvelopeType.noteSize */) || (legacyPulseEnv.type == 0 /* EnvelopeType.noteSize */);
        if (this.type == 1 /* InstrumentType.fm */) {
            noteSizeControlsSomethingElse = noteSizeControlsSomethingElse || (legacyFeedbackEnv.type == 0 /* EnvelopeType.noteSize */);
            for (let i = 0; i < legacyOperatorEnvelopes.length; i++) {
                if (i < carrierCount) {
                    if (legacyOperatorEnvelopes[i].type != 0 /* EnvelopeType.noteSize */) {
                    ...
                    noteSizeControlsSomethingElse = noteSizeControlsSomethingElse || (legacyOperatorEnvelopes[i].type == 0 /* EnvelopeType.noteSize */);
                }
            }
        }
        this.envelopeCount = 0;
        if (this.type == 1 /* InstrumentType.fm */) {
            if (allCarriersControlledByNoteSize && noteSizeControlsSomethingElse) {
                this.addEnvelope(SynthConfig_1.Config.instrumentAutomationTargets.dictionary["noteVolume"].index, 0, SynthConfig_1.Config.envelopes.dictionary["note size"].index);
            }
            else if (noCarriersControlledByNoteSize && !noteSizeControlsSomethingElse) {
                this.addEnvelope(SynthConfig_1.Config.instrumentAutomationTargets.dictionary["none"].index, 0, SynthConfig_1.Config.envelopes.dictionary["note size"].index);
            }
        }
        if (legacyFilterEnv.type == 1 /* EnvelopeType.none */) {
            this.noteFilter.reset();
            ...
            this.effects |= 1 << 5 /* EffectType.noteFilter */;
            this.addEnvelope(SynthConfig_1.Config.instrumentAutomationTargets.dictionary["noteFilterAllFreqs"].index, 0, legacyFilterEnv.index);
        }
        if (legacyPulseEnv.type != 1 /* EnvelopeType.none */) {
            this.addEnvelope(SynthConfig_1.Config.instrumentAutomationTargets.dictionary["pulseWidth"].index, 0, legacyPulseEnv.index);
        }
        for (let i = 0; i < legacyOperatorEnvelopes.length; i++) {
            if (i < carrierCount && allCarriersControlledByNoteSize)
                continue;
            if (legacyOperatorEnvelopes[i].type != 1 /* EnvelopeType.none */) {
                this.addEnvelope(SynthConfig_1.Config.instrumentAutomationTargets.dictionary["operatorAmplitude"].index, i, legacyOperatorEnvelopes[i].index);
            }
        }
        if (legacyFeedbackEnv.type != 1 /* EnvelopeType.none */) {
            this.addEnvelope(SynthConfig_1.Config.instrumentAutomationTargets.dictionary["feedbackAmplitude"].index, 0, legacyFeedbackEnv.index);
        }
    }
    toJsonObject() {
        ...
        const effects = [];
        for (const effect of SynthConfig_1.Config.effectOrder) {
        ...
        if ((0, SynthConfig_1.effectsIncludeDetune)(this.effects)) {
            instrumentObject["detuneCents"] = Synth.detuneToCents(this.detune - SynthConfig_1.Config.detuneCenter);
        }
        ...
        if (this.type == 2 /* InstrumentType.noise */) {
            ...
            for (let j = 0; j < SynthConfig_1.Config.drumCount; j++) {
                const spectrum = [];
                for (let i = 0; i < SynthConfig_1.Config.spectrumControlPoints; i++) {
            ...
            instrumentObject["unison"] = SynthConfig_1.Config.unisons[this.unison].name;
        }
        else if (this.type == 6 /* InstrumentType.pwm */) {
            instrumentObject["pulseWidth"] = Math.round((0, SynthConfig_1.getPulseWidthRatio)(this.pulseWidth) * 100 * 100000) / 100000;
        }
    ...
    fromJsonObject(instrumentObject, isNoiseChannel, legacyGlobalReverb = 0) {
        if (instrumentObject == undefined)
            instrumentObject = {};
        let type = SynthConfig_1.Config.instrumentTypeNames.indexOf(instrumentObject["type"]);
        if (type == -1)
            type = isNoiseChannel ? 2 /* InstrumentType.noise */ : 0 /* InstrumentType.chip */;
        ...
        if (Array.isArray(instrumentObject["effects"])) {
            ...
            this.effects = (effects & ((1 << 12 /* EffectType.length */) - 1));
        }
        else {
        ...
        if (transitionProperty != undefined) {
            let transition = SynthConfig_1.Config.transitions.dictionary[transitionProperty];
        ...
        }
        {
            ...
            const chord = SynthConfig_1.Config.chords.dictionary[legacyChordNames[chordProperty]] || SynthConfig_1.Config.chords.dictionary[chordProperty];
            if (chord != undefined) {
        ...
        if (unisonProperty != undefined) {
            ...
            const unison = SynthConfig_1.Config.unisons.dictionary[legacyChorusNames[unisonProperty]] || SynthConfig_1.Config.unisons.dictionary[unisonProperty];
            if (unison != undefined)
        ...
        if (instrumentObject["detuneCents"] != undefined) {
            this.detune = clamp(0, SynthConfig_1.Config.detuneMax + 1, Math.round(SynthConfig_1.Config.detuneCenter + Synth.centsToDetune(+instrumentObject["detuneCents"])));
        }
        ...
        if (vibratoProperty != undefined) {
            ...
            const vibrato = SynthConfig_1.Config.vibratos.dictionary[legacyVibratoNames[unisonProperty]] || SynthConfig_1.Config.vibratos.dictionary[vibratoProperty];
        ...
        if (instrumentObject["reverb"] != undefined) {
            ...
            if (legacyGlobalReverb == 0) {
                ...
                this.effects = (this.effects & (~(1 << 0 /* EffectType.reverb */)));
            }
            else {
        ...
        if (instrumentObject["pulseWidth"] != undefined) {
            this.pulseWidth = clamp(0, SynthConfig_1.Config.pulseWidthRange, Math.round(Math.log2((+instrumentObject["pulseWidth"]) / 50) / 0.5 - 1 + 8));
        }
        else {
            this.pulseWidth = SynthConfig_1.Config.pulseWidthRange - 1;
        }
        ...
        this.stringSustainType = SynthConfig_1.Config.sustainTypeNames.indexOf(instrumentObject["stringSustainType"]);
        if (this.stringSustainType == -1)
            this.stringSustainType = 0 /* SustainType.bright */;
        if (this.type == 2 /* InstrumentType.noise */) {
            this.chipNoise = SynthConfig_1.Config.chipNoises.findIndex(wave => wave.name == instrumentObject["wave"]);
            if (this.chipNoise == -1)
                this.chipNoise = 1;
        }
        const legacyEnvelopeNames = { "custom": "note size", "steady": "none", "pluck 1": "twang 1", "pluck 2": "twang 2", "pluck 3": "twang 3" };
        const getEnvelope = (name) => (legacyEnvelopeNames[name] != undefined) ? SynthConfig_1.Config.envelopes.dictionary[legacyEnvelopeNames[name]] : SynthConfig_1.Config.envelopes.dictionary[name];
        if (this.type == 4 /* InstrumentType.drumset */) {
        ...
        if (this.type == 0 /* InstrumentType.chip */) {
            ...
            this.chipWave = legacyWaveNames[instrumentObject["wave"]] != undefined ? legacyWaveNames[instrumentObject["wave"]] : SynthConfig_1.Config.chipWaves.findIndex(wave => wave.name == instrumentObject["wave"]);
            if (this.chipWave == -1)
                this.chipWave = 1;
        }
        if (this.type == 1 /* InstrumentType.fm */) {
            this.algorithm = SynthConfig_1.Config.algorithms.findIndex(algorithm => algorithm.name == instrumentObject["algorithm"]);
            if (this.algorithm == -1)
                this.algorithm = 0;
            this.feedbackType = SynthConfig_1.Config.feedbacks.findIndex(feedback => feedback.name == instrumentObject["feedbackType"]);
            ...
            for (let j = 0; j < SynthConfig_1.Config.operatorCount; j++) {
                ...
                if (operatorObject == undefined)
                    operatorObject = {};
                operator.frequency = SynthConfig_1.Config.operatorFrequencies.findIndex(freq => freq.name == operatorObject["frequency"]);
    ...
    supportsEnvelopeTarget(target, index) {
        const automationTarget = SynthConfig_1.Config.instrumentAutomationTargets[target];
    ...
    getTransition() {
        return (0, SynthConfig_1.effectsIncludeTransition)(this.effects) ? SynthConfig_1.Config.transitions[this.transition] : SynthConfig_1.Config.transitions.dictionary["normal"];
    }
    getFadeInSeconds() {
...
class Song {
    ...
    initToDefault(andResetChannels = true) {
        ...
        if (andResetChannels) {
            ...
            for (let channelIndex = 0; channelIndex < this.getChannelCount(); channelIndex++) {
                ...
                for (let instrument = 0; instrument < SynthConfig_1.Config.instrumentCountMin; instrument++) {
                    if (channel.instruments.length <= instrument) {
                        channel.instruments[instrument] = new Instrument(isNoiseChannel);
                    }
                    channel.instruments[instrument].setTypeAndReset(isNoiseChannel ? 2 /* InstrumentType.noise */ : 0 /* InstrumentType.chip */, isNoiseChannel);
                }
    ...
    toBase64String() {
        ...
        buffer.push(110 /* SongTagCode.channelCount */, base64IntToCharCode[this.pitchChannelCount], base64IntToCharCode[this.noiseChannelCount]);
        buffer.push(115 /* SongTagCode.scale */, base64IntToCharCode[this.scale]);
        buffer.push(107 /* SongTagCode.key */, base64IntToCharCode[this.key]);
        ...
        buffer.push(116 /* SongTagCode.tempo */, base64IntToCharCode[this.tempo >> 6], base64IntToCharCode[this.tempo & 63]);
        ...
        for (let channelIndex = 0; channelIndex < this.getChannelCount(); channelIndex++) {
            for (let i = 0; i < this.channels[channelIndex].instruments.length; i++) {
                ...
                buffer.push(118 /* SongTagCode.volume */, base64IntToCharCode[instrument.volume]);
                buffer.push(117 /* SongTagCode.preset */, base64IntToCharCode[instrument.preset >> 6], base64IntToCharCode[instrument.preset & 63]);
                buffer.push(102 /* SongTagCode.eqFilter */, base64IntToCharCode[instrument.eqFilter.controlPointCount]);
                ...
                buffer.push(113 /* SongTagCode.effects */, base64IntToCharCode[instrument.effects >> 6], base64IntToCharCode[instrument.effects & 63]);
                ...
                if (instrument.type == 5 /* InstrumentType.harmonics */ || instrument.type == 7 /* InstrumentType.pickedString */) {
                    ...
                    for (let i = 0; i < SynthConfig_1.Config.harmonicsControlPoints; i++) {
                        harmonicsBits.write(SynthConfig_1.Config.harmonicsControlPointBits, instrument.harmonicsWave.harmonics[i]);
                    }
                    harmonicsBits.encodeBase64(buffer);
                }
                if (instrument.type == 0 /* InstrumentType.chip */) {
                    buffer.push(119 /* SongTagCode.wave */, base64IntToCharCode[instrument.chipWave]);
                    buffer.push(104 /* SongTagCode.unison */, base64IntToCharCode[instrument.unison]);
                }
                else if (instrument.type == 1 /* InstrumentType.fm */) {
                    ...
                    for (let o = 0; o < SynthConfig_1.Config.operatorCount; o++) {
                    ...
                    for (let i = 0; i < SynthConfig_1.Config.spectrumControlPoints; i++) {
                        spectrumBits.write(SynthConfig_1.Config.spectrumControlPointBits, instrument.spectrumWave.spectrum[i]);
                    }
                    ...
                    for (let j = 0; j < SynthConfig_1.Config.drumCount; j++) {
                        for (let i = 0; i < SynthConfig_1.Config.spectrumControlPoints; i++) {
                            spectrumBits.write(SynthConfig_1.Config.spectrumControlPointBits, instrument.drumsetSpectrumWaves[j].spectrum[i]);
                        }
                    }
                    spectrumBits.encodeBase64(buffer);
                }
        ...
        const bitsPerNoteSize = Song.getNeededBits(SynthConfig_1.Config.noteSizeMax);
        for (let channelIndex = 0; channelIndex < this.getChannelCount(); channelIndex++) {
            const channel = this.channels[channelIndex];
            const maxInstrumentsPerPattern = this.getMaxInstrumentsPerPattern(channelIndex);
            const neededInstrumentCountBits = Song.getNeededBits(maxInstrumentsPerPattern - SynthConfig_1.Config.instrumentCountMin);
            ...
            const octaveOffset = isNoiseChannel ? 0 : channel.octave * SynthConfig_1.Config.pitchesPerOctave;
            ...
            for (const pattern of channel.patterns) {
                if (this.patternInstruments) {
                    const instrumentCount = validateRange(SynthConfig_1.Config.instrumentCountMin, maxInstrumentsPerPattern, pattern.instruments.length);
    ...
    fromBase64String(compressed) {
        ...
        if (beforeNine) {
            ...
            for (let i = legacySettingsCache.length; i < this.getChannelCount(); i++) {
                ...
                for (let j = 0; j < SynthConfig_1.Config.instrumentCountMin; j++)
        ...
        while (charIndex < compressed.length)
            switch (command = compressed.charCodeAt(charIndex++)) {
                ...
                case 105 /* SongTagCode.instrumentCount */:
                    {
                        if (beforeNine) {
                            const instrumentsPerChannel = validateRange(SynthConfig_1.Config.instrumentCountMin, SynthConfig_1.Config.patternInstrumentCountMax, base64CharCodeToInt[compressed.charCodeAt(charIndex++)] + SynthConfig_1.Config.instrumentCountMin);
                            this.layeredInstruments = false;
                            ...
                            for (let channelIndex = 0; channelIndex < this.getChannelCount(); channelIndex++) {
                                ...
                                if (beforeSix) {
                                    for (let instrumentIndex = 0; instrumentIndex < instrumentsPerChannel; instrumentIndex++) {
                                        this.channels[channelIndex].instruments[instrumentIndex].setTypeAndReset(isNoiseChannel ? 2 /* InstrumentType.noise */ : 0 /* InstrumentType.chip */, isNoiseChannel);
                                    }
                                }
                ...
                case 84 /* SongTagCode.startInstrument */:
                    {
                        ...
                        const instrumentType = validateRange(0, 8 /* InstrumentType.length */ - 1, base64CharCodeToInt[compressed.charCodeAt(charIndex++)]);
                ...
                case 102 /* SongTagCode.eqFilter */:
                    ...
                    break;
                case 121 /* SongTagCode.filterResonance */:
                    {
                        if (beforeNine) {
                            const filterResonanceRange = 8;
                            const instrument = this.channels[instrumentChannelIterator].instruments[instrumentIndexIterator];
                ...
                case 73 /* SongTagCode.stringSustain */:
                    {
                        ...
                        instrument.stringSustainType = clamp(0, 2 /* SustainType.length */, sustainValue >> 5);
                    }
                    break;
                        ...
                        if (beforeNine) {
                            if (beforeSeven) {
                                if (beforeThree) {
                                    ...
                                    if (legacySettings.filterEnvelope == undefined || legacySettings.filterEnvelope.type == 1 /* EnvelopeType.none */) {
                                ...
                                const instrument = this.channels[instrumentChannelIterator].instruments[instrumentIndexIterator];
                                const vibrato = clamp(0, SynthConfig_1.Config.vibratos.length, base64CharCodeToInt[compressed.charCodeAt(charIndex++)]);
                ...
                case 104 /* SongTagCode.unison */:
                    {
                        if (beforeThree) {
                            const channelIndex = base64CharCodeToInt[compressed.charCodeAt(charIndex++)];
                            this.channels[channelIndex].instruments[0].unison = clamp(0, SynthConfig_1.Config.unisons.length, base64CharCodeToInt[compressed.charCodeAt(charIndex++)]);
                        }
                        else if (beforeSix) {
                            for (let channelIndex = 0; channelIndex < this.getChannelCount(); channelIndex++) {
                                for (const instrument of this.channels[channelIndex].instruments) {
                                    const originalValue = base64CharCodeToInt[compressed.charCodeAt(charIndex++)];
                                    let unison = clamp(0, SynthConfig_1.Config.unisons.length, originalValue);
                                    if (originalValue == 8) {
                            ...
                            this.channels[instrumentChannelIterator].instruments[instrumentIndexIterator].unison = clamp(0, SynthConfig_1.Config.unisons.length, base64CharCodeToInt[compressed.charCodeAt(charIndex++)]);
                        }
                    }
                    break;
                ...
                case 113 /* SongTagCode.effects */:
                    {
                        const instrument = this.channels[instrumentChannelIterator].instruments[instrumentIndexIterator];
                        if (beforeNine) {
                            instrument.effects = (base64CharCodeToInt[compressed.charCodeAt(charIndex++)] & ((1 << 12 /* EffectType.length */) - 1));
                            if (legacyGlobalReverb == 0) {
                            ...
                            if (instrument.pan != SynthConfig_1.Config.panCenter) {
                            ...
                            if (12 /* EffectType.length */ > 12)
                                throw new Error();
                            instrument.effects = (base64CharCodeToInt[compressed.charCodeAt(charIndex++)] << 6) | (base64CharCodeToInt[compressed.charCodeAt(charIndex++)]);
                            if ((0, SynthConfig_1.effectsIncludeNoteFilter)(instrument.effects)) {
                                const originalControlPointCount = base64CharCodeToInt[compressed.charCodeAt(charIndex++)];
                                instrument.noteFilter.controlPointCount = clamp(0, SynthConfig_1.Config.filterMaxPoints + 1, originalControlPointCount);
                            ...
                            if ((0, SynthConfig_1.effectsIncludeDetune)(instrument.effects)) {
                                instrument.detune = clamp(0, SynthConfig_1.Config.detuneMax + 1, base64CharCodeToInt[compressed.charCodeAt(charIndex++)]);
                            }
                            if ((0, SynthConfig_1.effectsIncludeVibrato)(instrument.effects)) {
                                instrument.vibrato = clamp(0, SynthConfig_1.Config.vibratos.length, base64CharCodeToInt[compressed.charCodeAt(charIndex++)]);
                            }
                            if ((0, SynthConfig_1.effectsIncludeDistortion)(instrument.effects)) {
                        ...
                        // Clamp the range.
                        instrument.effects &= (1 << 12 /* EffectType.length */) - 1;
                    }
                    break;
                ...
                case 69 /* SongTagCode.envelopes */:
                    {
                        ...
                        if (beforeNine) {
                            ...
                            const envelopeCount = clamp(0, SynthConfig_1.Config.maxEnvelopeCount + 1, base64CharCodeToInt[compressed.charCodeAt(charIndex++)]);
                            for (let i = 0; i < envelopeCount; i++) {
                                const target = clamp(0, SynthConfig_1.Config.instrumentAutomationTargets.length, base64CharCodeToInt[compressed.charCodeAt(charIndex++)]);
                                let index = 0;
                                const maxCount = SynthConfig_1.Config.instrumentAutomationTargets[target].maxCount;
                                if (maxCount > 1) {
                                    index = clamp(0, maxCount, base64CharCodeToInt[compressed.charCodeAt(charIndex++)]);
                                }
                                const envelope = clamp(0, SynthConfig_1.Config.envelopes.length, base64CharCodeToInt[compressed.charCodeAt(charIndex++)]);
                ...
                case 83 /* SongTagCode.spectrum */:
                    {
                        ...
                        if (instrument.type == 3 /* InstrumentType.spectrum */) {
                            const byteCount = Math.ceil(SynthConfig_1.Config.spectrumControlPoints * SynthConfig_1.Config.spectrumControlPointBits / 6);
                            ...
                            const byteCount = Math.ceil(SynthConfig_1.Config.drumCount * SynthConfig_1.Config.spectrumControlPoints * SynthConfig_1.Config.spectrumControlPointBits / 6);
                            const bits = new BitFieldReader(compressed, charIndex, charIndex + byteCount);
                            for (let j = 0; j < SynthConfig_1.Config.drumCount; j++) {
                                for (let i = 0; i < SynthConfig_1.Config.spectrumControlPoints; i++) {
                ...
                case 72 /* SongTagCode.harmonics */:
                    {
                        const instrument = this.channels[instrumentChannelIterator].instruments[instrumentIndexIterator];
                        const byteCount = Math.ceil(SynthConfig_1.Config.harmonicsControlPoints * SynthConfig_1.Config.harmonicsControlPointBits / 6);
                        const bits = new BitFieldReader(compressed, charIndex, charIndex + byteCount);
                        for (let i = 0; i < SynthConfig_1.Config.harmonicsControlPoints; i++) {
                        ...
                        while (true) {
                            ...
                            for (let i = 0; i < this.patternsPerChannel; i++) {
                                ...
                                if (beforeNine) {
                                    ...
                                    if (this.patternInstruments) {
                                        const instrumentCount = validateRange(SynthConfig_1.Config.instrumentCountMin, maxInstrumentsPerPattern, bits.read(neededInstrumentCountBits) + SynthConfig_1.Config.instrumentCountMin);
                                        for (let j = 0; j < instrumentCount; j++) {
    ...
    fromJsonObject(jsonObject) {
        ...
        if (jsonObject["scale"] != undefined) {
            ...
            const scale = SynthConfig_1.Config.scales.findIndex(scale => scale.name == scaleName);
            if (scale != -1)
                this.scale = scale;
        }
        ...
        if (jsonObject["ticksPerBeat"] != undefined) {
            ...
            this.rhythm = SynthConfig_1.Config.rhythms.findIndex(rhythm => rhythm.stepsPerBeat == importedPartsPerBeat);
            if (this.rhythm == -1) {
                this.rhythm = 1;
            }
        }
...
class PickedString {
    ...
    update(synth, instrumentState, tone, stringIndex, roundedSamplesPerTick, stringDecayStart, stringDecayEnd, sustainType) {
        const allPassCenter = 2.0 * Math.PI * SynthConfig_1.Config.pickedStringDispersionCenterFreq / synth.samplesPerSecond;
        ...
        const allPassRadiansStart = Math.min(Math.PI, radiansPerSampleStart * SynthConfig_1.Config.pickedStringDispersionFreqMult * Math.pow(allPassCenter / radiansPerSampleStart, SynthConfig_1.Config.pickedStringDispersionFreqScale));
        const allPassRadiansEnd = Math.min(Math.PI, radiansPerSampleEnd * SynthConfig_1.Config.pickedStringDispersionFreqMult * Math.pow(allPassCenter / radiansPerSampleEnd, SynthConfig_1.Config.pickedStringDispersionFreqScale));
        const shelfRadians = 2.0 * Math.PI * SynthConfig_1.Config.pickedStringShelfHz / synth.samplesPerSecond;
        ...
        if (brightnessType == 0 /* PickedStringBrightnessType.bright */) {
            const shelfGainStart = Math.pow(decayRateStart, SynthConfig_1.Config.stringDecayRate);
            const shelfGainEnd = Math.pow(decayRateEnd, SynthConfig_1.Config.stringDecayRate);
...
class EnvelopeComputer {
    constructor( /*private _perNote: boolean*/) {
        ...
        const length = 33 /* EnvelopeComputeIndex.length */;
    ...
    computeEnvelopes(instrument, currentPart, tickTimeStart, secondsPerTick, tone) {
        ...
        const beatsPerTick = 1.0 / (SynthConfig_1.Config.ticksPerPart * SynthConfig_1.Config.partsPerBeat);
        ...
        if (tone != null && tone.note != null && !tone.passedEndOfNote) {
            ...
            const startPinTick = (tone.note.start + startPin.time) * SynthConfig_1.Config.ticksPerPart;
            const endPinTick = (tone.note.start + endPin.time) * SynthConfig_1.Config.ticksPerPart;
            ...
            if (transition.slides) {
                const noteStartTick = tone.noteStartPart * SynthConfig_1.Config.ticksPerPart;
                const noteEndTick = tone.noteEndPart * SynthConfig_1.Config.ticksPerPart;
        ...
        for (let envelopeIndex = 0; envelopeIndex <= instrument.envelopeCount; envelopeIndex++) {
            ...
            if (envelopeIndex == instrument.envelopeCount) {
                ...
                targetIndex = 0;
                envelope = SynthConfig_1.Config.envelopes.dictionary["note size"];
            }
            else {
                ...
                envelope = SynthConfig_1.Config.envelopes[envelopeSettings.envelope];
                if (envelope.type == 0 /* EnvelopeType.noteSize */)
                    usedNoteSize = true;
            }
            if ( /*automationTarget.perNote == this._perNote &&*/automationTarget.computeIndex != null) {
                ...
                if (automationTarget.isFilter) {
                    const filterSettings = /*this._perNote ?*/ instrument.noteFilter /*: instrument.eqFilter*/;
    ...
    static computeEnvelope(envelope, time, beats, noteSize) {
        switch (envelope.type) {
            case 0 /* EnvelopeType.noteSize */: return Synth.noteSizeToVolumeMult(noteSize);
            case 1 /* EnvelopeType.none */: return 1.0;
            case 4 /* EnvelopeType.twang */: return 1.0 / (1.0 + time * envelope.speed);
            case 5 /* EnvelopeType.swell */: return 1.0 - 1.0 / (1.0 + time * envelope.speed);
            case 6 /* EnvelopeType.tremolo */: return 0.5 - Math.cos(beats * 2.0 * Math.PI * envelope.speed) * 0.5;
            case 7 /* EnvelopeType.tremolo2 */: return 0.75 - Math.cos(beats * 2.0 * Math.PI * envelope.speed) * 0.25;
            case 2 /* EnvelopeType.punch */: return Math.max(1.0, 2.0 - time * 10.0);
            case 3 /* EnvelopeType.flare */:
                const attack = 0.25 / Math.sqrt(envelope.speed);
                return time < attack ? time / attack : 1.0 / (1.0 + (time - attack) * envelope.speed);
            case 8 /* EnvelopeType.decay */: return Math.pow(2, -envelope.speed * time);
    ...
    static getLowpassCutoffDecayVolumeCompensation(envelope) {
        ...
        // volume overall.
        if (envelope.type == 8 /* EnvelopeType.decay */)
            return 1.25 + 0.025 * envelope.speed;
        if (envelope.type == 4 /* EnvelopeType.twang */)
...
class Tone {
    constructor() {
        this.pitches = Array(SynthConfig_1.Config.maxChordSize).fill(0);
        this.pitchCount = 0;
        this.chordSize = 0;
    ...
    reset() {
        this.noiseSample = 0.0;
        for (let i = 0; i < SynthConfig_1.Config.maxPitchOrOperatorCount; i++) {
            this.phases[i] = 0.0;
...
class InstrumentState {
    ...
    allocateNecessaryBuffers(synth, instrument, samplesPerTick) {
        ...
        if ((0, SynthConfig_1.effectsIncludeEcho)(instrument.effects)) {
            ...
            const safeEchoDelaySteps = Math.max(SynthConfig_1.Config.echoDelayRange >> 1, (instrument.echoDelay + 1)); // The delay may be very short now, but if it increases later make sure we have enough sample history.
            const baseEchoDelayBufferSize = Synth.fittingPowerOfTwo(safeEchoDelaySteps * SynthConfig_1.Config.echoDelayStepTicks * samplesPerTick);
    ...
    compute(synth, instrument, samplesPerTick, roundedSamplesPerTick, tone) {
        ...
        if (instrument.distortion == 0)
            effects &= ~(1 << 3 /* EffectType.distortion */);
        if (instrument.pan == SynthConfig_1.Config.panCenter)
            effects &= ~(1 << 2 /* EffectType.panning */);
        if (instrument.chorus == 0)
            effects &= ~(1 << 1 /* EffectType.chorus */);
        if (instrument.echoSustain == 0)
            effects &= ~(1 << 6 /* EffectType.echo */);
        if (instrument.reverb == 0)
            effects &= ~(1 << 0 /* EffectType.reverb */);
        ...
        const usesDistortion = (0, SynthConfig_1.effectsIncludeDistortion)(effects);
        const usesBitcrusher = (0, SynthConfig_1.effectsIncludeBitcrusher)(effects);
        const usesPanning = (0, SynthConfig_1.effectsIncludePanning)(effects);
        const usesChorus = (0, SynthConfig_1.effectsIncludeChorus)(effects);
        const usesEcho = (0, SynthConfig_1.effectsIncludeEcho)(effects);
        const usesReverb = (0, SynthConfig_1.effectsIncludeReverb)(effects);
        if (usesDistortion) {
            const distortionSliderStart = Math.min(1.0, /*envelopeStarts[InstrumentAutomationIndex.distortion] **/ instrument.distortion / (SynthConfig_1.Config.distortionRange - 1));
            const distortionSliderEnd = Math.min(1.0, /*envelopeEnds[  InstrumentAutomationIndex.distortion] **/ instrument.distortion / (SynthConfig_1.Config.distortionRange - 1));
            ...
            const distortionDriveStart = (1.0 + 2.0 * distortionSliderStart) / SynthConfig_1.Config.distortionBaseVolume;
            const distortionDriveEnd = (1.0 + 2.0 * distortionSliderEnd) / SynthConfig_1.Config.distortionBaseVolume;
        ...
        if (usesBitcrusher) {
            const freqSettingStart = instrument.bitcrusherFreq /** Math.sqrt(envelopeStarts[InstrumentAutomationIndex.bitcrusherFrequency])*/;
            const freqSettingEnd = instrument.bitcrusherFreq /** Math.sqrt(envelopeEnds[  InstrumentAutomationIndex.bitcrusherFrequency])*/;
            const quantizationSettingStart = instrument.bitcrusherQuantization /** Math.sqrt(envelopeStarts[InstrumentAutomationIndex.bitcrusherQuantization])*/;
            const quantizationSettingEnd = instrument.bitcrusherQuantization /** Math.sqrt(envelopeEnds[  InstrumentAutomationIndex.bitcrusherQuantization])*/;
            const basePitch = SynthConfig_1.Config.keys[synth.song.key].basePitch; // TODO: What if there's a key change mid-song?
            const freqStart = Instrument.frequencyFromPitch(basePitch + 60) * Math.pow(2.0, (SynthConfig_1.Config.bitcrusherFreqRange - 1 - freqSettingStart) * SynthConfig_1.Config.bitcrusherOctaveStep);
            const freqEnd = Instrument.frequencyFromPitch(basePitch + 60) * Math.pow(2.0, (SynthConfig_1.Config.bitcrusherFreqRange - 1 - freqSettingEnd) * SynthConfig_1.Config.bitcrusherOctaveStep);
            ...
            const scaleStart = 2.0 * SynthConfig_1.Config.bitcrusherBaseVolume * Math.pow(2.0, 1.0 - Math.pow(2.0, (SynthConfig_1.Config.bitcrusherQuantizationRange - 1 - quantizationSettingStart) * 0.5));
            const scaleEnd = 2.0 * SynthConfig_1.Config.bitcrusherBaseVolume * Math.pow(2.0, 1.0 - Math.pow(2.0, (SynthConfig_1.Config.bitcrusherQuantizationRange - 1 - quantizationSettingEnd) * 0.5));
            this.bitcrusherScale = scaleStart;
            this.bitcrusherScaleScale = Math.pow(scaleEnd / scaleStart, 1.0 / roundedSamplesPerTick);
            const foldLevelStart = 2.0 * SynthConfig_1.Config.bitcrusherBaseVolume * Math.pow(1.5, SynthConfig_1.Config.bitcrusherQuantizationRange - 1 - quantizationSettingStart);
            const foldLevelEnd = 2.0 * SynthConfig_1.Config.bitcrusherBaseVolume * Math.pow(1.5, SynthConfig_1.Config.bitcrusherQuantizationRange - 1 - quantizationSettingEnd);
        ...
        for (let i = 0; i < eqFilterSettings.controlPointCount; i++) {
            ...
            this.eqFilters[i].loadCoefficientsWithGradient(Synth.tempFilterStartCoefficients, Synth.tempFilterEndCoefficients, 1.0 / roundedSamplesPerTick, point.type == 0 /* FilterType.lowPass */);
        ...
        if (usesPanning) {
            ...
            const pan = (instrument.pan - SynthConfig_1.Config.panCenter) / SynthConfig_1.Config.panCenter;
            const panStart = Math.max(-1.0, Math.min(1.0, pan /** panEnvelopeStart*/));
            const panEnd = Math.max(-1.0, Math.min(1.0, pan /** panEnvelopeEnd  */));
            ...
            const maxDelaySamples = samplesPerSecond * SynthConfig_1.Config.panDelaySecondsMax;
        ...
        if (usesChorus) {
            ...
            let chorusStart = Math.min(1.0, /*chorusEnvelopeStart **/ instrument.chorus / (SynthConfig_1.Config.chorusRange - 1));
            let chorusEnd = Math.min(1.0, /*chorusEnvelopeEnd   **/ instrument.chorus / (SynthConfig_1.Config.chorusRange - 1));
        ...
        if (usesEcho) {
            ...
            const echoMultStart = Math.min(1.0, Math.pow(/*echoSustainEnvelopeStart **/ instrument.echoSustain / SynthConfig_1.Config.echoSustainRange, 1.1)) * 0.9;
            const echoMultEnd = Math.min(1.0, Math.pow(/*echoSustainEnvelopeEnd   **/ instrument.echoSustain / SynthConfig_1.Config.echoSustainRange, 1.1)) * 0.9;
            ...
            const echoDelayOffset = Math.round((instrument.echoDelay + 1) * SynthConfig_1.Config.echoDelayStepTicks * samplesPerTick);
            ...
            const shelfRadians = 2.0 * Math.PI * SynthConfig_1.Config.echoShelfHz / synth.samplesPerSecond;
        ...
        if (usesReverb) {
            ...
            const reverbStart = Math.min(1.0, Math.pow(/*reverbEnvelopeStart **/ instrument.reverb / SynthConfig_1.Config.reverbRange, 0.667)) * 0.425;
            const reverbEnd = Math.min(1.0, Math.pow(/*reverbEnvelopeEnd   **/ instrument.reverb / SynthConfig_1.Config.reverbRange, 0.667)) * 0.425;
            this.reverbMult = reverbStart;
            this.reverbMultDelta = (reverbEnd - reverbStart) / roundedSamplesPerTick;
            maxReverbMult = Math.max(reverbStart, reverbEnd);
            const shelfRadians = 2.0 * Math.PI * SynthConfig_1.Config.reverbShelfHz / synth.samplesPerSecond;
        ...
        if (this.tonesAddedInThisTick) {
            ...
            if (usesReverb) {
                ...
                const averageReverbDelaySeconds = (SynthConfig_1.Config.reverbDelayBufferSize / 4.0) / samplesPerSecond;
    ...
    updateWaves(instrument, samplesPerSecond) {
        if (instrument.type == 0 /* InstrumentType.chip */) {
            this.wave = SynthConfig_1.Config.chipWaves[instrument.chipWave].samples;
        }
        else if (instrument.type == 2 /* InstrumentType.noise */) {
...
class Synth {
    ...
    constructor(activeAudio, song = null) {
                ...
                for (let instrumentIndex = 0; instrumentIndex < channel.instruments.length; instrumentIndex++) {
                    ...
                    if (this.isAtStartOfTick) {
                        ...
                        for (let i = 0; i < instrumentState.releasedTones.count(); i++) {
                            ...
                            const shouldFadeOutFast = (tonesPlayedInThisInstrument >= SynthConfig_1.Config.maximumTonesPerChannel);
    ...
    determineCurrentActiveTones(song, channelIndex, samplesPerTick, playSong) {
        ...
        const currentTick = this.tick + SynthConfig_1.Config.ticksPerPart * currentPart;
        let note = null;
        let prevNote = null;
        let nextNote = null;
        ...
        for (let instrumentIndex = 0; instrumentIndex < channel.instruments.length; instrumentIndex++) {
            ...
            if ((note != null) && (!song.patternInstruments || (pattern.instruments.indexOf(instrumentIndex) != -1))) {
                ...
                const partsPerBar = SynthConfig_1.Config.partsPerBeat * song.beatsPerBar;
                const transition = instrument.getTransition();
                ...
                if (chord.singleTone) {
                    const atNoteStart = (SynthConfig_1.Config.ticksPerPart * note.start == currentTick);
                    let tone;
                    if (toneList.count() <= toneCount) {
                        tone = this.newTone();
                        toneList.pushBack(tone);
                    ...
                    const transition = instrument.getTransition();
                    if (((transition.isSeamless && !transition.slides && chord.strumParts == 0) || forceContinueAtStart) && (SynthConfig_1.Config.ticksPerPart * note.start == currentTick) && prevNoteForThisInstrument != null) {
                    ...
                    for (let i = 0; i < note.pitches.length; i++) {
                        ...
                        if ((transition.isSeamless || forceContinueAtStart) && nextNoteForThisTone != null) {
                            noteEndPart = Math.min(SynthConfig_1.Config.partsPerBeat * this.song.beatsPerBar, noteEndPart + strumOffsetParts);
                        }
                        if ((!transition.continues && !forceContinueAtStart) || prevNoteForThisTone == null) {
                            strumOffsetParts += chord.strumParts;
                        }
                        const atNoteStart = (SynthConfig_1.Config.ticksPerPart * noteStartPart == currentTick);
                        let tone;
                        if (this.tempMatchedPitchTones[toneCount] != null) {
                            tone = this.tempMatchedPitchTones[toneCount];
                            this.tempMatchedPitchTones[toneCount] = null;
    ...
    computeTone(song, channelIndex, samplesPerTick, tone, released, shouldFadeOutFast) {
        ...
        const intervalScale = isNoiseChannel ? SynthConfig_1.Config.noiseInterval : 1;
        const secondsPerPart = SynthConfig_1.Config.ticksPerPart * samplesPerTick / this.samplesPerSecond;
        const sampleTime = 1.0 / this.samplesPerSecond;
        const beatsPerPart = 1.0 / SynthConfig_1.Config.partsPerBeat;
        const ticksIntoBar = this.getTicksIntoBar();
        const partTimeStart = (ticksIntoBar) / SynthConfig_1.Config.ticksPerPart;
        const partTimeEnd = (ticksIntoBar + 1.0) / SynthConfig_1.Config.ticksPerPart;
        const currentPart = this.getCurrentPart();
        ...
        let basePitch = SynthConfig_1.Config.keys[song.key].basePitch;
        ...
        if (released) {
            ...
            const noteStartTick = noteStartPart * SynthConfig_1.Config.ticksPerPart;
            const noteEndTick = noteEndPart * SynthConfig_1.Config.ticksPerPart;
            const pinStart = (note.start + startPin.time) * SynthConfig_1.Config.ticksPerPart;
            const pinEnd = (note.start + endPin.time) * SynthConfig_1.Config.ticksPerPart;
            tone.ticksSinceReleased = 0;
            const tickTimeStart = currentPart * SynthConfig_1.Config.ticksPerPart + this.tick;
            const tickTimeEnd = tickTimeStart + 1.0;
        ...
        envelopeComputer.computeEnvelopes(instrument, currentPart, SynthConfig_1.Config.ticksPerPart * partTimeStart, samplesPerTick / this.samplesPerSecond, tone);
        ...
        if ((0, SynthConfig_1.effectsIncludePitchShift)(instrument.effects)) {
            const pitchShift = SynthConfig_1.Config.justIntonationSemitones[instrument.pitchShift] / intervalScale;
            const envelopeStart = envelopeStarts[14 /* EnvelopeComputeIndex.pitchShift */];
            const envelopeEnd = envelopeEnds[14 /* EnvelopeComputeIndex.pitchShift */];
            intervalStart += pitchShift * envelopeStart;
            intervalEnd += pitchShift * envelopeEnd;
        }
        if ((0, SynthConfig_1.effectsIncludeDetune)(instrument.effects)) {
            const envelopeStart = envelopeStarts[15 /* EnvelopeComputeIndex.detune */];
            const envelopeEnd = envelopeEnds[15 /* EnvelopeComputeIndex.detune */];
            intervalStart += Synth.detuneToCents((instrument.detune - SynthConfig_1.Config.detuneCenter) * envelopeStart) * SynthConfig_1.Config.pitchesPerOctave / (12.0 * 100.0);
            intervalEnd += Synth.detuneToCents((instrument.detune - SynthConfig_1.Config.detuneCenter) * envelopeEnd) * SynthConfig_1.Config.pitchesPerOctave / (12.0 * 100.0);
        }
        if ((0, SynthConfig_1.effectsIncludeVibrato)(instrument.effects)) {
            const delayTicks = SynthConfig_1.Config.vibratos[instrument.vibrato].delayTicks;
            const vibratoAmplitude = SynthConfig_1.Config.vibratos[instrument.vibrato].amplitude;
            ...
            if (tone.prevVibrato != null) {
                ...
                const vibratoDepthEnvelopeStart = envelopeStarts[16 /* EnvelopeComputeIndex.vibratoDepth */];
            ...
            const vibratoDepthEnvelopeEnd = envelopeEnds[16 /* EnvelopeComputeIndex.vibratoDepth */];
        ...
        if (!(0, SynthConfig_1.effectsIncludeNoteFilter)(instrument.effects)) {
            ...
            for (let i = 0; i < noteFilterSettings.controlPointCount; i++) {
                const noteFreqEnvelopeStart = envelopeStarts[17 /* EnvelopeComputeIndex.noteFilterFreq0 */ + i];
                const noteFreqEnvelopeEnd = envelopeEnds[17 /* EnvelopeComputeIndex.noteFilterFreq0 */ + i];
                const notePeakEnvelopeStart = envelopeStarts[25 /* EnvelopeComputeIndex.noteFilterGain0 */ + i];
                const notePeakEnvelopeEnd = envelopeEnds[25 /* EnvelopeComputeIndex.noteFilterGain0 */ + i];
                ...
                tone.noteFilters[i].loadCoefficientsWithGradient(Synth.tempFilterStartCoefficients, Synth.tempFilterEndCoefficients, 1.0 / roundedSamplesPerTick, point.type == 0 /* FilterType.lowPass */);
        ...
        if (instrument.type == 1 /* InstrumentType.fm */) {
            ...
            if (tone.pitchCount > 1 && arpeggiates) {
                const arpeggio = Math.floor((this.tick + this.part * SynthConfig_1.Config.ticksPerPart) / SynthConfig_1.Config.rhythms[song.rhythm].ticksPerArpeggio);
                arpeggioInterval = tone.pitches[(0, SynthConfig_1.getArpeggioPitchIndex)(tone.pitchCount, song.rhythm, arpeggio)] - tone.pitches[0];
            }
            const carrierCount = SynthConfig_1.Config.algorithms[instrument.algorithm].carrierCount;
            for (let i = 0; i < SynthConfig_1.Config.operatorCount; i++) {
                const associatedCarrierIndex = SynthConfig_1.Config.algorithms[instrument.algorithm].associatedCarrier[i] - 1;
                ...
                const freqMult = SynthConfig_1.Config.operatorFrequencies[instrument.operators[i].frequency].mult;
                const interval = SynthConfig_1.Config.operatorCarrierInterval[associatedCarrierIndex] + arpeggioInterval;
                ...
                const hzOffset = SynthConfig_1.Config.operatorFrequencies[instrument.operators[i].frequency].hzOffset;
                ...
                const amplitudeMult = amplitudeCurve * SynthConfig_1.Config.operatorFrequencies[instrument.operators[i].frequency].amplitudeSign;
                ...
                if (i < carrierCount) {
                    ...
                    // modulator
                    expressionStart *= SynthConfig_1.Config.sineWaveLength * 1.5;
                    expressionEnd *= SynthConfig_1.Config.sineWaveLength * 1.5;
                    sineExpressionBoost *= 1.0 - Math.min(1.0, instrument.operators[i].amplitude / 15);
                }
                expressionStart *= envelopeStarts[9 /* EnvelopeComputeIndex.operatorAmplitude0 */ + i];
                expressionEnd *= envelopeEnds[9 /* EnvelopeComputeIndex.operatorAmplitude0 */ + i];
            ...
            const expressionStart = baseExpression * sineExpressionBoost * noteFilterExpression * fadeExpressionStart * chordExpressionStart * envelopeStarts[0 /* EnvelopeComputeIndex.noteVolume */];
            const expressionEnd = baseExpression * sineExpressionBoost * noteFilterExpression * fadeExpressionEnd * chordExpressionEnd * envelopeEnds[0 /* EnvelopeComputeIndex.noteVolume */];
            tone.expression = expressionStart;
            tone.expressionDelta = (expressionEnd - expressionStart) / roundedSamplesPerTick;
            const feedbackAmplitude = SynthConfig_1.Config.sineWaveLength * 0.3 * instrument.feedbackAmplitude / 15.0;
            let feedbackStart = feedbackAmplitude * envelopeStarts[13 /* EnvelopeComputeIndex.feedbackAmplitude */];
            let feedbackEnd = feedbackAmplitude * envelopeEnds[13 /* EnvelopeComputeIndex.feedbackAmplitude */];
            ...
            if (tone.pitchCount > 1 && (chord.arpeggiates || chord.customInterval)) {
                ...
                if (chord.customInterval) {
                    const intervalOffset = tone.pitches[1 + (0, SynthConfig_1.getArpeggioPitchIndex)(tone.pitchCount - 1, song.rhythm, arpeggio)] - tone.pitches[0];
                    specialIntervalMult = Math.pow(2.0, intervalOffset / 12.0);
                    tone.specialIntervalExpressionMult = Math.pow(2.0, -intervalOffset / pitchDamping);
                }
                else {
                    pitch = tone.pitches[(0, SynthConfig_1.getArpeggioPitchIndex)(tone.pitchCount, song.rhythm, arpeggio)];
                }
            }
            ...
            if (instrument.type == 6 /* InstrumentType.pwm */) {
                const basePulseWidth = (0, SynthConfig_1.getPulseWidthRatio)(instrument.pulseWidth);
                const pulseWidthStart = basePulseWidth * envelopeStarts[2 /* EnvelopeComputeIndex.pulseWidth */];
                const pulseWidthEnd = basePulseWidth * envelopeEnds[2 /* EnvelopeComputeIndex.pulseWidth */];
                tone.pulseWidth = pulseWidthStart;
                tone.pulseWidthDelta = (pulseWidthEnd - pulseWidthStart) / roundedSamplesPerTick;
            }
            if (instrument.type == 7 /* InstrumentType.pickedString */) {
                // Increase expression to compensate for string decay.
                settingsExpressionMult *= Math.pow(2.0, 0.7 * (1.0 - instrument.stringSustain / (SynthConfig_1.Config.stringSustainRange - 1)));
            }
            const startFreq = Instrument.frequencyFromPitch(startPitch);
            if (instrument.type == 0 /* InstrumentType.chip */ || instrument.type == 5 /* InstrumentType.harmonics */ || instrument.type == 7 /* InstrumentType.pickedString */) {
                // These instruments have two waves at different frequencies for the unison feature.
                const unison = SynthConfig_1.Config.unisons[instrument.unison];
                const voiceCountExpression = (instrument.type == 7 /* InstrumentType.pickedString */) ? 1 : unison.voices / 2.0;
            ...
            const expressionStart = settingsExpressionMult * fadeExpressionStart * chordExpressionStart * pitchExpressionStart * envelopeStarts[0 /* EnvelopeComputeIndex.noteVolume */];
            const expressionEnd = settingsExpressionMult * fadeExpressionEnd * chordExpressionEnd * pitchExpressionEnd * envelopeEnds[0 /* EnvelopeComputeIndex.noteVolume */];
            tone.expression = expressionStart;
            tone.expressionDelta = (expressionEnd - expressionStart) / roundedSamplesPerTick;
            if (instrument.type == 7 /* InstrumentType.pickedString */) {
                ...
                if (tone.prevStringDecay != null) {
                    ...
                    stringDecayStart = 1.0 - Math.min(1.0, sustainEnvelopeStart * instrument.stringSustain / (SynthConfig_1.Config.stringSustainRange - 1));
                }
                const sustainEnvelopeEnd = tone.envelopeComputer.envelopeEnds[3 /* EnvelopeComputeIndex.stringSustain */];
                let stringDecayEnd = 1.0 - Math.min(1.0, sustainEnvelopeEnd * instrument.stringSustain / (SynthConfig_1.Config.stringSustainRange - 1));
    ...
    static getLFOAmplitude(instrument, secondsIntoBar) {
        let effect = 0.0;
        for (const vibratoPeriodSeconds of SynthConfig_1.Config.vibratos[instrument.vibrato].periodsSeconds) {
            ...
            if (Synth.fmSynthFunctionCache[fingerprint] == undefined) {
                ...
                for (const line of Synth.fmSourceTemplate) {
                    if (line.indexOf("// CARRIER OUTPUTS") != -1) {
                        const outputs = [];
                        for (let j = 0; j < SynthConfig_1.Config.algorithms[instrument.algorithm].carrierCount; j++) {
                            outputs.push("operator" + j + "Scaled");
                        }
                        synthSource.push(line.replace("/*operator#Scaled*/", outputs.join(" + ")));
                    }
                    else if (line.indexOf("// INSERT OPERATOR COMPUTATION HERE") != -1) {
                        for (let j = SynthConfig_1.Config.operatorCount - 1; j >= 0; j--) {
                            for (const operatorLine of Synth.operatorSourceTemplate) {
                                if (operatorLine.indexOf("/* + operator@Scaled*/") != -1) {
                                    let modulators = "";
                                    for (const modulatorNumber of SynthConfig_1.Config.algorithms[instrument.algorithm].modulatedBy[j]) {
                                        modulators += " + operator" + (modulatorNumber - 1) + "Scaled";
                                    }
                                    const feedbackIndices = SynthConfig_1.Config.feedbacks[instrument.feedbackType].indices[j];
    ...
    static effectsSynth(synth, outputDataL, outputDataR, bufferIndex, runLength, instrumentState) {
        ...
        const usesDistortion = (0, SynthConfig_1.effectsIncludeDistortion)(instrumentState.effects);
        const usesBitcrusher = (0, SynthConfig_1.effectsIncludeBitcrusher)(instrumentState.effects);
        const usesEqFilter = instrumentState.eqFilterCount > 0;
        const usesPanning = (0, SynthConfig_1.effectsIncludePanning)(instrumentState.effects);
        const usesChorus = (0, SynthConfig_1.effectsIncludeChorus)(instrumentState.effects);
        const usesEcho = (0, SynthConfig_1.effectsIncludeEcho)(instrumentState.effects);
        const usesReverb = (0, SynthConfig_1.effectsIncludeReverb)(instrumentState.effects);
        let signature = 0;
        if (usesDistortion)
    ...
    static noiseSynth(synth, bufferIndex, runLength, tone, instrumentState) {
        ...
        let expression = +tone.expression;
        const expressionDelta = +tone.expressionDelta;
        let phase = (tone.phases[0] % 1) * SynthConfig_1.Config.chipNoiseLength;
        if (tone.phases[0] == 0) {
            // Zero phase means the tone was reset, just give noise a random start phase instead.
            phase = Math.random() * SynthConfig_1.Config.chipNoiseLength;
        }
        const phaseMask = SynthConfig_1.Config.chipNoiseLength - 1;
    ...
    static spectrumSynth(synth, bufferIndex, runLength, tone, instrumentState) {
        ...
        let initialFilterInput2 = +tone.initialNoteFilterInput2;
        const applyFilters = Synth.applyFilters;
        let phase = (tone.phases[0] % 1) * SynthConfig_1.Config.spectrumNoiseLength;
        // Zero phase means the tone was reset, just give noise a random start phase instead.
        if (tone.phases[0] == 0)
            phase = Synth.findRandomZeroCrossing(wave, SynthConfig_1.Config.spectrumNoiseLength) + phaseDelta;
        const phaseMask = SynthConfig_1.Config.spectrumNoiseLength - 1;
        // This is for a "legacy" style simplified 1st order lowpass filter with
        // a cutoff frequency that is relative to the tone's fundamental frequency.
        ...
        tone.phases[0] = phase / SynthConfig_1.Config.spectrumNoiseLength;
    ...
    static volumeMultToInstrumentVolume(volumeMult) {
        return (volumeMult <= 0.0) ? SynthConfig_1.Config.volumeRange - 1 : Math.min(SynthConfig_1.Config.volumeRange - 2, Math.log2(volumeMult) / SynthConfig_1.Config.volumeLogScale);
    }
    ...
    static ticksToFadeOutSetting(ticks) {
        let lower = SynthConfig_1.Config.fadeOutTicks[0];
        if (ticks <= lower)
            return 0;
        for (let i = 1; i < SynthConfig_1.Config.fadeOutTicks.length; i++) {
            let upper = SynthConfig_1.Config.fadeOutTicks[i];
            if (ticks <= upper)
    ...
    getSamplesPerTick() {
        ...
        const partsPerSecond = SynthConfig_1.Config.partsPerBeat * beatsPerSecond;
        const tickPerSecond = SynthConfig_1.Config.ticksPerPart * partsPerSecond;
...
Synth.fmSourceTemplate = (`
		...
		let operator#Phase       = +((tone.phases[#] % 1) + 1000) * ` + SynthConfig_1.Config.sineWaveLength + `;
